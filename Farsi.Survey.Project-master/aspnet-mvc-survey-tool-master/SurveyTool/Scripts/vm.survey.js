var SurveyModel = function(data) {
    ...
    // Functions
    self.saveChoice = function (item) {
        debugger;
        var index;
        if (item.isValid()) {
            index = self.Choices.indexOf(item);
            if (index >= 0) {
                self.Choices.splice(index, 1);
                /*Validation_Question*/
                //self.questions.splice(index, 1, item);
                self.Choices.splice(index, 0, item);
            } else {
                self.Choices.push(item);
            }

            self.modalChoices.modal('hide');
        }
        else {
    ...
    if (data != null) {
        for (var i = 0; i < data.Questions.length; i++) {
            ...
            q.type(data.Questions[i].Validation_Question);
